 ...
 */
void detect_objects(float sensor_data_array[]) {
    ...
    for (i = 0; i < NUM_OF_SCANS; i++) {
        ...
        next_distance = sensor_data_array[i];
        ...
        if (next_distance < limit_distance) {
            ...
            if (in_object == 't') {
                ...
                if ((next_distance - sensor_data_array[i-1] < delta_limit) && (sensor_data_array[i-1] - next_distance < delta_limit)) {
 ...
 */
void clean_scanner_data(float sensor_data_array[]) {
    float delta_limit = 15;                             // What delta do we consider as an outlier reading

    float current_distance;                             // Current value from the sensor_data_array

    int i;
    for (i = 4; i < NUM_OF_SCANS - 1; i++) {            // we process indexes from 1 to 89, as 0 and 90 don't have prev/next readings.
        current_distance = sensor_data_array[i];
        
        // 1. Clean 1: Check for outlier points and normalize them by averaging it to their neighbors
        if (((current_distance - sensor_data_array[i+1] > delta_limit) && (current_distance - sensor_data_array[i-1] > delta_limit)) ||
        ((sensor_data_array[i+1] - current_distance > delta_limit) && (sensor_data_array[i-1] - current_distance > delta_limit))) {
            
            current_distance = (sensor_data_array[i-1] + sensor_data_array[i+1]) / 2.0;
            sensor_data_array[i] = current_distance;
        }
    }

    sensor_data_array[0] = (sensor_data_array[3] + sensor_data_array[4] + sensor_data_array[5]) / 3.0f;
    sensor_data_array[1] = (sensor_data_array[3] + sensor_data_array[4] + sensor_data_array[5]) / 3.0f;
    sensor_data_array[2] = (sensor_data_array[3] + sensor_data_array[4] + sensor_data_array[5]) / 3.0f;
}


/*
 * Perform ultra-sonic scans on 180 degrees range and save distances to sensor_data_array.
 */
// void perform_scan(float sensor_data_array[], int angle_increment, cyBOT_Scan_t* scanStruct) {
//     int i, current_angle;
...
//        *(sensor_data_array + i) = scanStruct->sound_dist;  // Save measured distance to sensor_data_array
//     }
// }

// void send_scan_to_putty(float sensor_data_array[], int angle_increment) {
//     // Print header
...
//     for (i = 0; i < NUM_OF_SCANS; i++) {
//         sprintf(destination1, "%-11d%.1f\r\n", i * angle_increment, *(sensor_data_array + i));
...
int main(void) {
    ...
    // float sensor_data_array[91];                 // We use the array from the header file.
  
    // 1. We don't perform scan as the data is pre-defined.
    // perform_scan(sensor_data_array, angle_increment, &scanStruct);

    // 2. Clean the data.
    clean_scanner_data(sensor_data_array);

    // 3. Detect objects from the data and calculate their radial and liner width
    detect_objects(sensor_data_array);

    // 4. Print whole array and object structs to the terminal/putty, depending on the phase of development
    //send_scan_to_putty(sensor_data_array, angle_increment);
    display_scan_to_terminal();
